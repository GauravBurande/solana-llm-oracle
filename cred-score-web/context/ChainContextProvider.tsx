...
const STORAGE_KEY = "solana-cred-score-web:selected-chain";
...
export function ChainContextProvider({
  ...
  const [chain, setChain] = useState<`solana:${string}`>(() => {
    if (typeof window === "undefined") {
      return "solana:devnet";
    }
    return (
      (localStorage.getItem(STORAGE_KEY) as `solana:${string}` | null) ??
      "solana:devnet"
    );
  });
